prawn_document(filename: @filename) do |pdf|
  pdf.text 'GENDER BUDGET STATEMENT ', size: 16, align: :center
  # tabel pertama
  pdf.move_down 20
  tabel_program_kegiatan = [
    ['Perangkat Daerah', ':', { content: @program_kegiatan.opd.nama_opd, font_style: :bold }],
    ['Tahun', ':', { content: @tahun.to_s, font_style: :bold }],
    ['Kode Program', ':', { content: @program_kegiatan.kode_program, font_style: :bold }],
    ...
    ['', '', ''],
    ['Kode Kegiatan', ':', { content: @program_kegiatan.kode_giat, font_style: :bold }],
    ...
    ['', '', ''],
    ['Kode Sub Kegiatan', ':', { content: @program_kegiatan.kode_sub_giat, font_style: :bold }],
    ...
    ['Pagu Anggaran', ':', "Rp. #{number_with_delimiter(@program_kegiatan.my_pagu, delimiter: '.')}"]
  ]
  ...
  @program_kegiatan.sasarans.where.not(program_kegiatan: nil).each.with_index(1) do |sasaran, index|
    ...
    data_terpilah = [
      ['', 'a.', 'Data Pembuka Wawasan', ':', { content: sasaran.rincian&.data_terpilah || '-' }]
    ]
    permasalahan = [
      ['', 'b.', 'Isu dan Faktor Kesenjangan Gender', ':', permasalahan_cell]
    ]
    rencana_aksi_judul = [
      ['', 'c.', {content: 'Rencana Aksi dan Anggaran'}]
    ]
